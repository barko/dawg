...
let py_eval_function features trees model kind =
  ...
  let comments = [
      `Line (sp "# auto-generated by user %S with:" user_name);
...
let gen input_file_path output_file_path_opt positive_category_opt =
  let model =
    ...
    Model_j.c_model_of_string model_s
  in
    ...
    match model with
      ...
      | `Square _ ->
        match positive_category_opt with
          ...
          | None -> `Square
  in
  let code = py_eval_function features trees model kind in
