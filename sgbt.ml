...
module Make ( L : Loss.LOSS ) = struct
  ...
  let wrk_and_val_metrics splitter fold_set =
    let wrk_metrics =
      match splitter#metrics (fun i -> fold_set.(i)) with
        | Some metrics -> metrics
        | None -> assert false
    in
    let val_metrics =
      match splitter#metrics (fun i -> not fold_set.(i)) with
        | Some metrics -> metrics
        | None -> assert false
    in
    wrk_metrics, val_metrics
        ...
        match t.splitter#boost gamma with
          ...
          | `Ok ->
            ...
            pr "iter % 3d % 7d  %s %s    %+.4e %+.4e\n%!"
              iteration.fold
              iteration.i
              (L.string_of_metrics wrk_metrics)
              (L.string_of_metrics val_metrics)
              convergence_rate
  ...
  let learn conf =
    let dog_reader = Dog_io.create_reader conf.dog_file_path in
    ...
    let splitter = new L.splitter y_feature num_observations
      conf.map_target_opt in
    ...
    Dog_io.close_reader dog_reader
